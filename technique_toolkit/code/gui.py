...
class FairnessToolGUI(tk.Tk):
    ...
    def show_dashboard(self, results: List[RunResult]):
        ...
        # Small helpers (define once if not already in file)
        def _fmt(x): return "NA" if pd.isna(x) else f"{x:.3f}"
        def _fmt_delta(curr, base, *, invert=False):
            if pd.isna(curr) or pd.isna(base): return "NA"
            d = (curr - base)
            if invert: d = -d
            return f"{d:+.3f}"
        ...
        # ------------- Overview (responsive with scrollbars + Δ columns) -------------
        ...
        def _b(k): return float(base.get(k, np.nan)) if base else np.nan
        ...
        ttk.Label(
            overview,
            text="Δ vs Baseline: positive means improvement (Brier/ECE/gap metrics inverted so lower is better → positive Δ).",
            foreground="#555"
        ).grid(row=1, column=0, sticky="w", padx=8, pady=(0,8))

        #Auto-resize metric columns as the pane grows
        ...
        top = ttk.Frame(group_tab); top.grid(row=0, column=0, sticky="ew", padx=8, pady=8)
        ttk.Label(top, text="Technique:").pack(side="left")
        sel_var = tk.StringVar(value=results[0].name)
        sel_combo = ttk.Combobox(top, textvariable=sel_var, values=[r.name for r in results], state="readonly", width=60)
        sel_combo.pack(side="left", padx=8)

        g_cols = ["group","n","TPR","FPR","PPV","NPV","PPR","ECE"]
        g_frame = ttk.Frame(group_tab); g_frame.grid(row=1, column=0, sticky="nsew", padx=8, pady=(0,8))
        g_frame.rowconfigure(0, weight=1); g_frame.columnconfigure(0, weight=1)
        ...
        def refresh_group_table(name):
            ...
            for _, row in df.iterrows():
                g_tree.insert("", "end", values=[
                    row["group"], int(row["n"]),
                    _fmt(row["TPR"]), _fmt(row["FPR"]), _fmt(row["PPV"]), _fmt(row["NPV"]),
                    _fmt(row["PPR"]), _fmt(row["ECE"])
                ])
        ...
        pm = ttk.Frame(plot_tab); pm.grid(row=0, column=0, sticky="ew", padx=8, pady=8)
        ttk.Label(pm, text="Metric:").pack(side="left")
        metric_var = tk.StringVar(value="AUROC")
        metric_combo = ttk.Combobox(pm, textvariable=metric_var,
                                    values=["ACC","AUROC","AUPRC","F1","Brier","ECE","EO_diff"],
        ...
        def render_barplot(metric="AUROC", px=1000):
            fig_w = max(6.0, px/130.0)  # heuristic inches
