...
def run_combined_pipeline(model_name, params,
    ...
    if "Post:Multiaccuracy Boost" in post_plan and P_val is not None:
        #One hot encode group labels for validation
        Gv = pd.get_dummies(Ava, drop_first=False)
        #Construct residual feature matrix: [P_val | Gv] (P-val = original validation probabilities) (Gv = one-hot group indicators)
        Zv = np.column_stack([P_val] + [Gv[c].to_numpy() for c in Gv.columns])
        #Fit a logistic regression residual on validation set using group membership as features
        resid = LogisticRegression(max_iter=200)
        resid.fit(Zv, yva)
        #Construct test residual feature matrix: [p_test | Gt]
        Gt = pd.get_dummies(Ate, drop_first=False).reindex(columns=Gv.columns, fill_value=0)
        Zt = np.column_stack([p_test] + [Gt[c].to_numpy() for c in Gv.columns])
        #Predict boosted probabilities on test set replacing p_test
        p_test = resid.predict_proba(Zt)[:, 1]
